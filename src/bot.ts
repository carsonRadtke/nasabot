...
import { AcceptedEnrollment, SubscriptionManagerCleanup, SubscriptionManagerInitialize } from "./lib/subscriptionManager";
import { CommandKind, CommandResult, HandleCommand } from "./lib/commandHandler";

const Commands: { [command: string]: { description: string, handler: (result: CommandResult, message: Discord.Message) => Promise<void> } } = {}
...
async function ReplyWithEnrollment(_res: CommandResult, { channel }: Discord.Message) {
  if (CanSendOnChannel(channel)) {
    if (AcceptedEnrollment(channel.id)) {
      ...
      await channel.send("This channel is already subscribed to daily APOD updates.");
    }
  }
}
...
function InitializeCommands() {
  Commands[CommandKind.Subscribe] = { description: "Subscribe to daily APOD updates", handler: ReplyWithEnrollment };
  Commands[CommandKind.APOD] = { description: "Get the Astronomy Picture of the Day", handler: ReplyWithAPOD };
  Commands[CommandKind.Help] = { description: "Show this message", handler: ReplyWithHelp };
}
...
function Main() {
  ...
  Client.on(Discord.Events.MessageCreate, async (msg: Discord.Message) => {
    ...
    if (content.startsWith("./nasabot")) {
      ...
      await (Commands[command.kind] ?? Commands[CommandKind.Help]).handler(command, msg);
    }
  });
